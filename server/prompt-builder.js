...
Analyze the following patient-provided hip and knee evaluation data and generate a concise clinical summary.
...
- Relevant medical history (hip/knee diagnoses, surgeries, treatments), including specific details for "other" conditions, main symptoms, symptom duration, and progression.
...
`;
    ...
    if (formData.diagnoses) {
        ...
        if (formData.diagnoses.glutealTendonTear) hipConditions.push("Gluteal Tendon Tear");
        if (formData.diagnoses.hipStressFracture) hipConditions.push("Hip Stress Fracture");
        if (formData.diagnoses.avascularNecrosis) hipConditions.push("Avascular Necrosis");
    ...
    if (formData.surgeries && formData.surgeries.length > 0 && formData.hadSurgery) {
        ...
        comprehensivePrompt += "\nHip/Knee Surgical History: No surgical history reported.\n";
    }
